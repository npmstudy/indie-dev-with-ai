...
- 学习状态库，比如zustand、jotai等，我不是很喜欢redux这种
...
- 异步管理：TanStack Query | React Query, Solid Query, Svelte Query, Vue Query
